...
TokenFlux 提供 OpenAI 兼容格式的 API 中转服务。接入时通常只需要把请求地址改成 TokenFlux 的 OpenAI 兼容地址，并使用你在 TokenFlux 控制台创建的 API Key。
...
- TokenFlux API Key，例如 `sk-xxxxxxxxxxxxxxxxxxxxxxxx`
- 可用模型名称，例如你在 TokenFlux 控制台或模型列表里看到的模型名
...
const client = new OpenAI({
  apiKey: process.env.TOKENFLUX_API_KEY,
...
client = OpenAI(
    api_key=os.environ.get("TOKENFLUX_API_KEY"),
...
public class TokenFluxExample {
    public static void main(String[] args) throws Exception {
        String apiKey = System.getenv("TOKENFLUX_API_KEY");
...
$env:TOKENFLUX_API_KEY="sk-xxxxxxxxxxxxxxxxxxxxxxxx"
```

```bash [macOS / Linux]
export TOKENFLUX_API_KEY="sk-xxxxxxxxxxxxxxxxxxxxxxxx"
```
...
通常是模型名称不可用或拼写错误。请在 TokenFlux 控制台确认模型名称，然后把请求里的 `model` 改成实际可用的模型。
